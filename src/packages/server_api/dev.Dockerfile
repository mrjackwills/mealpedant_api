...
RUN mkdir /healthcheck /logs /photo_converted /photo_original \
	&& chown ${DOCKER_APP_USER}:${DOCKER_APP_GROUP} /app /healthcheck /logs /photo_converted /photo_original
...
COPY --chown=${DOCKER_APP_USER}:${DOCKER_APP_GROUP} ./health_server_api.sh /healthcheck/
RUN chmod +x /healthcheck/health_server_api.sh
