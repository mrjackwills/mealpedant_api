...
import { LOCATION_LOG_COMBINED, LOCATION_LOG_ERROR, MODE_ENV_DEV, MODE_ENV_TEST, SHOW_LOGS } from './env';
...
class PostgresTransport extends Transport {
	...
	async log (info: LogEntry, callback: () => void): Promise<void> {
		await sharedQueries.insert_error(info);
		callback();
	}
}
