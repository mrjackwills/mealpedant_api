...
abstract class RegexConstants extends Constants {
	...
	readonly regex_backupSQLOnly = /^mealpedant_\d{4}-\d{2}-\d{2}_\d{2}\.\d{2}\.\d{2}_LOGS_REDIS_SQL_[0-9a-fA-F]{8}\.tar\.gz\.gpg$/;
	readonly regex_backupFull = /^mealpedant_\d{4}-\d{2}-\d{2}_\d{2}\.\d{2}\.\d{2}_LOGS_PHOTOS_REDIS_SQL_[0-9a-fA-F]{8}\.tar\.gpg$/;
}
...
abstract class Helpers extends Responses {
	...
	get randomMessageName (): string {
		const choices = [ 'backup::sql_backup', 'backup::full_backup', 'ping' ];
