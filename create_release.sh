...
update_version_number_in_files() {
	...
	sed -i -r -E "s=location \/v[0-9]+\/=location /v${MAJOR}\/=g" ./nginx_confs/conf.d/api.mealpedant.com.conf

	# update nginx api config location with new major version - static - for internal auth checker
	sed -i -r -E "s=api_upstream/v[0-9]+=api_upstream/v${MAJOR}=g" ./nginx_confs/conf.d/static.mealpedant.com.conf
...
build_container_amd64() {
	echo -e "${YELLOW}docker build  --platform linux/amd64 --no-cache -t mealpedant_amd64 --no-cache -f ./docker/dockerfile/api.Dockerfile .; docker save -o /tmp/mealpedant_amd64.tar mealpedant_amd64${RESET}"
	docker build --platform linux/amd64 --no-cache -t mealpedant_amd64 -f ./docker/dockerfile/api.Dockerfile .
	docker save -o /tmp/mealpedant_amd64.tar mealpedant_amd64
}
# build container for aarm64 platform
build_container_arm64() {
	echo -e "${YELLOW}docker build  --platform linux/arm64 --no-cache -t mealpedant_arm64 --no-cache -f ./docker/dockerfile/api.Dockerfile .; docker save -o /tmp/mealpedant_arm64.tar mealpedant_arm64${RESET}"
	docker build --platform linux/arm64 --no-cache -t mealpedant_arm64 -f ./docker/dockerfile/api.Dockerfile .
	docker save -o /tmp/mealpedant_arm64.tar mealpedant_arm64
}
