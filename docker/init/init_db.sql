...
CREATE DATABASE mealpedant WITH OWNER "$DB_USER";
...
\ c mealpedant CREATE TABLE IF NOT EXISTS ip_address (
...
GRANT USAGE,
SELECT
	ON SEQUENCE ip_address_ip_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE user_agent_user_agent_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE registered_user_registered_user_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE admin_user_admin_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE login_attempt_login_attempt_id_seq TO mealpedant;
...
CREATE TABLE IF NOT EXISTS password_reset (
	...
	reset_string TEXT NOT NULL CHECK(LENGTH(reset_string) = 256) ip_id BIGINT REFERENCES ip_address(ip_id) NOT NULL,
	user_agent_id BIGINT REFERENCES user_agent(user_agent_id) NOT NULL,
...
GRANT USAGE,
SELECT
	ON SEQUENCE password_reset_password_reset_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_category_meal_category_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_description_meal_description_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_person_meal_person_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_photo_meal_photo_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_date_meal_date_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE individual_meal_individual_meal_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE login_history_login_history_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE two_fa_secret_two_fa_secret_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE two_fa_backup_two_fa_backup_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE banned_email_domain_banned_email_domain_id_seq TO mealpedant;
...
CREATE TABLE IF NOT EXISTS email_log (
	...
	email TEXT NOT NULL,
	registered_user_id BIGINT REFERENCES registered_user(registered_user_id) ON DELETE
	SET
		NULL,
...
GRANT USAGE,
SELECT
	ON SEQUENCE email_log_email_log_id_seq TO mealpedant;
...
GRANT USAGE,
SELECT
	ON SEQUENCE error_log_error_log_id_seq TO mealpedant;

-- * Function to minus jsonb from jsonb, works with nested jsons as well!
CREATE
OR REPLACE FUNCTION jsonb_minus (arg1 jsonb, arg2 jsonb) RETURNS jsonb AS $ $
SELECT
	COALESCE(
		json_object_agg(
			key,
			CASE
				...
				AND arg2 -> key IS NOT NULL THEN jsonb_minus(value, arg2 -> key)
				ELSE value
			END
		),
		'{}'
...
WHERE
	arg1 -> key <> arg2 -> key
	OR arg2 -> key IS NULL $ $ LANGUAGE SQL;
...
GRANT USAGE,
SELECT
	ON SEQUENCE registered_user_audit_user_audit_id_seq TO mealpedant;

-- * create function, to be executed by trigger, to insert into user_audit table
CREATE FUNCTION registered_user_modified_func() RETURNS TRIGGER AS $ body $ BEGIN IF tg_op = 'UPDATE' THEN
INSERT into
...
INSERT into
	registered_user_audit (table_name, user_name, action, new_values, query)
VALUES
	(
		tg_table_name :: TEXT,
		current_user,
...
CREATE TRIGGER user_audit_trig BEFORE
INSERT
	OR
UPDATE
	OR DELETE ON registered_user FOR EACH ROW EXECUTE PROCEDURE registered_user_modified_func();
...
CREATE FUNCTION individual_meal_modify_func() RETURNS TRIGGER AS $ body $ BEGIN IF tg_op = 'UPDATE' THEN
INSERT into
	individual_meal_audit (
		table_name,
		user_name,
		action,
		old_values,
...
ELSIF tg_op = 'INSERT' THEN
INSERT into
	individual_meal_audit (table_name, user_name, action, new_values, query)
VALUES
	(
		tg_table_name :: TEXT,
		current_user,
...
CREATE TRIGGER individual_meal_audit_trig BEFORE
INSERT
	OR
UPDATE
	OR DELETE ON individual_meal FOR EACH ROW EXECUTE PROCEDURE individual_meal_modify_func();
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_category_audit_meal_category_audit_id_seq TO mealpedant;

CREATE FUNCTION meal_category_modify_func() RETURNS TRIGGER AS $ body $ BEGIN IF tg_op = 'UPDATE' THEN
INSERT into
	meal_category_audit (
		table_name,
		user_name,
...
ELSIF tg_op = 'INSERT' THEN
INSERT into
	meal_category_audit (table_name, user_name, action, new_values, query)
VALUES
	(
		tg_table_name :: TEXT,
		current_user,
...
CREATE TRIGGER meal_category_audit_trig BEFORE
INSERT
	OR
UPDATE
	OR DELETE ON meal_category FOR EACH ROW EXECUTE PROCEDURE meal_category_modify_func();
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_date_audit_meal_date_audit_id_seq TO mealpedant;

CREATE FUNCTION meal_date_modify_func() RETURNS TRIGGER AS $ body $ BEGIN IF tg_op = 'UPDATE' THEN
INSERT into
	meal_date_audit (
		table_name,
		user_name,
...
ELSIF tg_op = 'INSERT' THEN
INSERT into
	meal_date_audit (table_name, user_name, action, new_values, query)
VALUES
	(
		tg_table_name :: TEXT,
		current_user,
...
CREATE TRIGGER meal_date_audit_trig BEFORE
INSERT
	OR
UPDATE
	OR DELETE ON meal_date FOR EACH ROW EXECUTE PROCEDURE meal_date_modify_func();
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_description_audit_meal_description_audit_id_seq TO mealpedant;

CREATE FUNCTION meal_description_modify_func() RETURNS TRIGGER AS $ body $ BEGIN IF tg_op = 'UPDATE' THEN
INSERT into
	meal_description_audit (
		table_name,
		user_name,
		action,
		old_values,
...
ELSIF tg_op = 'INSERT' THEN
INSERT into
	meal_description_audit (table_name, user_name, action, new_values, query)
VALUES
	(
		tg_table_name :: TEXT,
		current_user,
...
CREATE TRIGGER meal_description_audit_trig BEFORE
INSERT
	OR
UPDATE
	OR DELETE ON meal_description FOR EACH ROW EXECUTE PROCEDURE meal_description_modify_func();
...
GRANT USAGE,
SELECT
	ON SEQUENCE meal_photo_audit_meal_photo_audit_id_seq TO mealpedant;

CREATE FUNCTION meal_photo_modify_func() RETURNS TRIGGER AS $ body $ BEGIN IF tg_op = 'UPDATE' THEN
INSERT into
	meal_photo_audit (
		table_name,
		user_name,
...
ELSIF tg_op = 'INSERT' THEN
INSERT into
	meal_photo_audit (table_name, user_name, action, new_values, query)
VALUES
	(
		tg_table_name :: TEXT,
		current_user,
...
CREATE TRIGGER meal_photo_audit_trig BEFORE
INSERT
	OR
UPDATE
	OR DELETE ON meal_photo FOR EACH ROW EXECUTE PROCEDURE meal_photo_modify_func();